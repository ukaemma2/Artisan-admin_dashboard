...
export const getServerSideProps = async ({ params: { artizansRecord }, req, }) => {
  const token = parseCookies(req);
  ...
  try {
    const { data } = await authAxios(token).get(
      `/admins/artizans/${artizansRecord}`
    );
    props.artizan = data;
  } catch (error) {
    throw error;
...
function ArtizanProfiles({ artizan }) {
        ...
        <CardBody>
          ...
          <TableContainer component={Paper}>
            <Table className={classess.table} aria-label="customized table">
              ...
              <TableBody>
                <StyledTableRow className={classess.data} key={artizan._id}>
                  <StyledTableCell align="right">
                    {artizan.first_name}
                  </StyledTableCell>
                  <StyledTableCell align="right">
                  ...
                  <StyledTableCell align="right">
                    {artizan.phone_number}
                  </StyledTableCell>
                  <StyledTableCell align="right">
                    {artizan.category_id}
                  </StyledTableCell>
                  <StyledTableCell align="right">
                    {artizan.short_description}
                  </StyledTableCell>
                  <StyledTableCell align="right">
                    {artizan.address}
                  </StyledTableCell>
                  <StyledTableCell align="right">
                    {artizan.certifications}
                  </StyledTableCell>
                </StyledTableRow>
              </TableBody>
            </Table>
